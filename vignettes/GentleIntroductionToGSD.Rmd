...
There is no programming shown, but by accessing the source for the article all required programming can be accessed; substantial commenting is provided in the source in the hope that users can understand how to implement the concepts.
There is substantial additional documentation for those wishing to know more.
...
- a new treatment for patients with unstable angina undergoing balloon angioplasty with a positive interim finding for a binary outcome of death, myocardial infarction or urgent repeat intervention within 30 days (@CAPTURE).
...
- $k$ analyses are planned for some integer $k> 1.$
- A natural parameter $\delta$ that has an asymptotically normal and efficient estimate $\hat\delta_j$ with variance $\sigma_j^2$ and corresponding statistical information $\mathcal{I}_j=1/\sigma_j^2$, at analysis $j=1,2,\ldots,k$. The parameter $\delta$ denotes an underlying treatment difference between treatment groups with a positive value favoring experimental treatment and negative value favoring control. We assume a consistent estimate $\hat\sigma_j^2$ of $\sigma_j^2, j=1,2,\ldots,k$.
- Information fraction $t_j=\mathcal{I}_i/\mathcal{I}_j$ at analysis $j=1,\ldots,k$.
- Correlations 
$\hbox{Corr}(\hat\delta_i,\hat\delta_j)=\sqrt{\mathcal{I}_i/\mathcal{I}_j}=\sqrt{t_j}$ 
for $1\le i\le j\le k.$
- A test $Z_j\approx\hat\delta_j/\hat{\sigma}^2_j.$
...
$\delta_1>0$ which represents a benefit of experimental treatment compared to cotnrol.
...
sd <- 9.123 # Standard deviation for change in HAM-D score
...
The actual observations at the analysis were 90 as opposed to the planned 98, which resulted in interim spending fraction $t_1=$ `r round(90 / x$n.I[2], 4)`.
With the Lan-DeMets spending function to approximate O'Brien-Fleming bounds this results in 
$\alpha$-spending of
`r round(sfLDOF(alpha = alpha, t = 90 / x$n.I[2])$spend, 4)` (`P(Cross) if delta=0` row in Efficacy column)
and $\beta$-spending of 
`r round(sfLDOF(alpha = beta, t = 90 / x$n.I[2])$spend, 4)` (`P(Cross) if delta=3` row in Futility column).
...
We note further that at the final analysis, the cumulative probability for `P(Cross) if delta=0` is less than the planned $\alpha=0.10$. This probability represents $\alpha(0)$ which excludes the probability of crossing the lower bound at the interim analysis and the final analysis. 
