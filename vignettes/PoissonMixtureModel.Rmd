...
In particular, we design consider two alternatives to the usual event-based spending:

- calendar-based timing and spending @LanDeMets1989, and
- spending based on the minimum of planned and actual observed events.

We discuss some of the potential advantages and disadvantages of these approaches in this application where hazard rates for events decrease substantially over time and the true underlying distributions may meaningfully deviate from what is anticipated at the time of design.
...
```{r}
# Control group assumptions for two Poisson mixture cure models
cure_rate <- c(.5, .35, .55)
...
We create the following functions to support examples below. Their use is demonstrated below, but users are referred to the vignette markdown source if code is needed.
...
survival <- rbind(survival,
     tibble(Scenario = scenario, Treatment = "Control",      Time = t, 
            Survival = pPM(x = t, cure_rate = cure_rate[scenario],      
                           t1 = t1[scenario], s1 = s1[scenario])),
     tibble(Scenario = scenario, Treatment = "Experimental", Time = t, 
            Survival = pPM(x = t, cure_rate = cure_rate[scenario] ^ hr[scenario], 
                           t1 = t1[scenario], s1 = s1[scenario] ^ hr[scenario]))
     )
}
survival <- survival %>% mutate(Scenario = as.factor(Scenario))
ggplot(survival, aes(x=Time, y=Survival, lty=Treatment, col=Scenario))+ geom_point() + 
...
ggplot() +
  geom_step(data = hazard %>% filter(Treatment == "Control", Scenario == 1), 
            aes(x = time_lagged, y = hazard_rate)) +
  ylab("Hazard rate") +
  xlab("Time") +
  ggtitle("Step Function Approximated Hazard Rate for Design Cure Model (Control Group)",
...
for(scenario in 1:length(cure_rate)){
   ...
   for(T in 1:(study_duration[1]+12)){
      xc <- eEvents(lambda = control$hazard_rate, S = (control$Time - control$time_lagged)[1:(bins-1)], 
      ...
      xe <- eEvents(lambda = control$hazard_rate * hr[scenario], S = (control$Time - control$time_lagged)[1:(bins-1)], 
                    gamma = 100 / enroll_duration[scenario], R = enroll_duration[scenario], eta = dropout_rate[scenario], 
                    T =  T, Tfinal = study_duration[1])
...
ggplot(event_accrual, aes(x = Time, y = EF, color = Scenario, lty = Hypothesis)) +
  ...
  ggtitle("Fraction of planned events expected over time by scenario",
...
We choose calendar timing deliberately. For the example, we choose
...
We begin with a design using event-based spending, the default for gsSurv().
...
```{r}
design %>% gsBoundSummary(exclude = c("B-value", "CP", "CP H1", "PP")) %>% gt()
```
...
# Expected behavior for scenario 3
