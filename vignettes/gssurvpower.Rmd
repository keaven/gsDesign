...
- **`informationRates`**: Planned information fractions (vector of
  ...
  and under-spending when behind.

- **`fullSpendingAtFinal`**: When `TRUE`, the spending fraction at the
  final analysis is forced to 1 regardless of the actual information
  fraction. This ensures full alpha spending even when expected events
  fall short of the design target.
...
spending based on planned information fractions.

Setting `fullSpendingAtFinal = TRUE` forces the spending fraction
at the final analysis to 1, ensuring full alpha is spent even when
expected events fall short of the design target. Without this,
the final spending fraction would be less than 1, leaving some
alpha unspent.
...
data.frame(
  ...
  Planned_InfoFrac = round(design$timing, 3),
  Spending_Frac = round(
    pmin(design$timing, pwr_slow_ir$timing), 3
  )
)
...
even though the actual information fraction is less than 1. This
produces slightly different final bounds compared to the default,
which would use the actual (sub-1) fraction for spending.
