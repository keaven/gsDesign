        ...
        ui(new Ui::gsDesign)
{
    ...
    ui->spendingFunctionTab->setCurrentIndex(0); // spending function lower
    ...
    ui->ssBinRatioDSpin->setMinimum(0.001);
    ui->ssBinRatioDSpin->setMaximum(1000.0);
    ui->ssBinRatioDSpin->setSingleStep(0.5);;

    ui->ssBinControlDSpin->setMinimum(0.0001);
    ui->ssBinControlDSpin->setMaximum(0.9999);
    ui->ssBinExpDSpin->setMinimum(0.0001);
    ui->ssBinExpDSpin->setMaximum(0.9999);

    setDeltaRange();
    ui->ssBinDeltaDSpin->setSingleStep(0.1);
    ...
    ui->ssTECtrlDSpin->setMinimum(0.0001);
    ui->ssTEExpDSpin->setMinimum(0.0001);
    ui->ssTEDropoutDSpin->setMinimum(0.0001);

    ui->ssTECtrlDSpin->setMaximum(1e6);
    ui->ssTEExpDSpin->setMaximum(1e6);
    ui->ssTEDropoutDSpin->setMaximum(1e6);

    ui->ssTEAccrualDSpin->setMinimum(0.0001);
    ui->ssTEFollowDSpin->setMinimum(0.0001);
    ui->ssTERatioDSpin->setMinimum(0.0001);

    ui->ssTEGammaDSpin->setMinimum(0.0001);
    ...
    ui->sfl2PPt1XDSpin->setSingleStep(pointStep);
    ui->sfl2PPt2XDSpin->setSingleStep(pointStep);
    ui->sfl2PPt1YDSpin->setSingleStep(pointStep);
    ui->sfl2PPt2YDSpin->setSingleStep(pointStep);

    ui->sfl3PPt1XDSpin->setSingleStep(pointStep);
    ui->sfl3PPt2XDSpin->setSingleStep(pointStep);
    ui->sfl3PPt1YDSpin->setSingleStep(pointStep);
    ui->sfl3PPt2YDSpin->setSingleStep(pointStep);

    // upper spending 2- and 3-parameter X & Y singleStep size
    ui->sfu2PPt1XDSpin->setSingleStep(pointStep);
    ui->sfu2PPt2XDSpin->setSingleStep(pointStep);
    ui->sfu2PPt1YDSpin->setSingleStep(pointStep);
    ui->sfu2PPt2YDSpin->setSingleStep(pointStep);

    ui->sfu3PPt1XDSpin->setSingleStep(pointStep);
    ui->sfu3PPt2XDSpin->setSingleStep(pointStep);
    ui->sfu3PPt1YDSpin->setSingleStep(pointStep);
    ui->sfu3PPt2YDSpin->setSingleStep(pointStep);
    ...
    ui->sfl2PLMSlpDSpin->setMinimum(0.001);
    ui->sfu2PLMSlpDSpin->setMinimum(0.001);
    ui->sfl3PLMSlpDSpin->setMinimum(0.001);
    ui->sfu3PLMSlpDSpin->setMinimum(0.001);
    ...
    ui->sfl2PLMIntDSpin->setMinimum(-1e6);
    ui->sfl2PLMIntDSpin->setMaximum(1e6);
    ui->sfl2PLMSlpDSpin->setValue(0.68);
    ui->sfl2PLMIntDSpin->setValue(-0.07);

    ui->sfl3PLMIntDSpin->setMinimum(-1e6);
    ui->sfl3PLMIntDSpin->setMaximum(1e6);
    ui->sfl3PLMSlpDSpin->setValue(0.68);
    ui->sfl3PLMIntDSpin->setValue(-0.07);

    ui->sfu2PLMIntDSpin->setMinimum(-1e6);
    ui->sfu2PLMIntDSpin->setMaximum(1e6);
    ui->sfu2PLMSlpDSpin->setValue(0.68);
    ui->sfu2PLMIntDSpin->setValue(-0.07);

    ui->sfu3PLMIntDSpin->setMinimum(-1e6);
    ui->sfu3PLMIntDSpin->setMaximum(1e6);
    ...
    // QDoubleSpinBox
    foreach (QDoubleSpinBox *dspin, ui->gsDesignMainWidget->findChildren<QDoubleSpinBox*>()) {
    ...
    ui->anlMaxnIPlanDSpin->setMinimum(1.0);
    ui->anlMaxnIPlanDSpin->setMaximum((double) 1e6);
...
void gsDesign::initialize1ParamSpending(QComboBox *combo, QDoubleSpinBox *param, gsDesign::Spending spending)
{
    ...
    case LowerSpending:
        ...
        {
            param->setMinimum(-40.0);
            param->setMaximum(40.0);
            param->setValue(-1.0);
        }

        if (type == "Power")
        {
            param->setMinimum(0.0001);
            param->setMaximum(DBLMAX);
            param->setValue(2.0);
        }
...
void gsDesign::on_dnNameCombo_textChanged(QString string)
{
    ui->dnNameCombo->blockSignals(true);
...
void gsDesign::on_dnDescCombo_textChanged(QString )
{
    updateDesignMapQComboBox(ui->dnDescCombo);
}
...
void gsDesign::setPowerMinimum()
{
    ...
    {
        ui->eptPowerDSpin->setMinimum(error + delta);
    }
    else
    {
        ui->eptPowerDSpin->setMinimum(100.0);
    }
}
...
void gsDesign::on_eptPowerDSpin_valueChanged()
{
    ...
    ui->eptPowerDSpin->setMinimum(ui->eptErrorDSpin->value() + 0.0001);
}
...
void gsDesign::setDeltaRange()
{
    ...
    double tol = 0.001;
    ui->ssBinDeltaDSpin->setMinimum(-pC + tol);
    ui->ssBinDeltaDSpin->setMaximum(1 - pC - tol);
}
...
void gsDesign::updateTimeToEventValues()
{
    ...
    QDoubleSpinBox *control = ui->ssTECtrlDSpin;
    QDoubleSpinBox *experimental = ui->ssTEExpDSpin;
    QDoubleSpinBox *dropout = ui->ssTEDropoutDSpin;
...
void gsDesign::setSpendingPointsBounds(QDoubleSpinBox *dspin1, QDoubleSpinBox *dspin2)
{
    ...
    dspin1->setMinimum(min1);
    dspin1->setMaximum(max1);
    dspin2->setMinimum(min2);
    dspin2->setMaximum(max2);
}
...
QString gsDesign::QDoubleSpinBoxValueToQString(QDoubleSpinBox *dspin)
{
...
void gsDesign::updateDesignMapQDoubleSpinBox(QDoubleSpinBox *dspin)
{
    if (!designList.isEmpty())
    {
        int index = ui->dnNameCombo->currentIndex();
...
void gsDesign::designMapToQDoubleSpinBox(QDoubleSpinBox *dspin)
{
    int index = ui->dnNameCombo->currentIndex();
    QMap<QString, QString> design = designList[index];
...
void gsDesign::on_dnNameCombo_currentIndexChanged(int )
{
    loadDesign();
