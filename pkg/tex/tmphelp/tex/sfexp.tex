...
The function \code{sfExponential} implements an investigational (unpublished) spending function called the exponential
spending function.
...
\deqn{\alpha(t)=\alpha^{t^{-\nu}}.}{alpha(t)=alpha^(t^(-nu)).}
...
\deqn{\alpha(t)=1-F\left(F^{-1}(1-\alpha)/ t^\nu\right).}{alpha(t)=1-F(F^(-1)(1-alpha)/ t^nu).}
...
\deqn{\alpha(t)=2-2\Phi \left( \Phi^{-1}(1-\alpha/2)/ t^{1/2} \right).}{alpha(t)=2-2*Phi(Phi^(-1)(1-alpha/2)/t^(1/2)).}
\end{Details}
\begin{Value}
An object of type \code{spendfn}. See \LinkA{Spending Functions}{Spending Functions} for further details.
\end{Value}
\begin{Note}\relax
The manual shows how to closely approximate an O'Brien-Fleming design using \code{sfExponential()}.
