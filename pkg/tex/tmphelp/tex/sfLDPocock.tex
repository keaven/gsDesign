\HeaderA{sfLDOF}{4.4: Lan-DeMets Spending Functions}{sfLDOF}
...
O'Brien-Fleming bound is implemented in (\code{sfLDOF}):
\deqn{\alpha(t)=2-2\Phi\left(\Phi^{-1}(1-\alpha/2)/ t^{1/2}\right).}{alpha(t)=2-2*Phi(Phi^(-1)(1-alpha/2)/t^(1/2)\right).}
The Lan-DeMets spending function to approximate a Pocock design is implemented in {\code{sfLDPocock}}:
\deqn{\alpha(t)=ln(1+(e-1)t).}{alpha(t)=ln(1+(e-1)t).}
