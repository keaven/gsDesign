...
{	if (!(class(x)=="gsProbability" || class(x)=="gsDesign"))
	{	gsReturnError(xnew,1,"gsCP must be called with class of x either gsProbability or gsDesign")
	}
	if (i<1 || i>=x$k)
	{	gsReturnError(xnew,2,"gsCP must be called with i from 1 to x$k-1")
	}
	if (class(x)=="gsProbability") test.type<-3
	else test.type<-x$test.type
	if (zi > x$upper$bound[i])
	{	gsReturnError(xnew,3,"gsCP must have x$lower$bound[i]<=zi<=x$upper$bound[i]")
	}
	else if (test.type>1)
	{	if (zi<x$lower$bound[i])
			gsReturnError(xnew,3,"gsCP must have x$lower$bound[i]<=zi<=x$upper$bound[i]")
	}
	if (!is.real(theta)) theta=c(zi/sqrt(x$n.I[i]),x$theta)
	knew<-x$k-i
	...
	else anew<-array(-20,knew)
	return(gsProbability(k=knew,theta=theta,n.I=Inew,a=anew,b=bnew,r=r))
}
gsBoundCP<-function(x,theta="thetahat",r=18)
{	len<-x$k-1
	if (class(x)=="gsProbability") test.type<-3
	else test.type<-x$test.type
	if (class(x)!="gsDesign" || theta!="thetahat")
	{	thetahi<-array(theta,len)
		if (test.type>1) thetalow<-theta	
	}
	else
	...
	if (test.type>1) return(cbind(CPlo,CPhi))
	else return(CPhi)
}
