...
# check n.I, maxn.IPlan
	...
	if (max(abs(x$n.I))>0)
	{	if (length(x$n.I)<2) return(gsReturnError(x,errcode=11.2,errmsg="n.I must be 0 or length must be >1"))
		if (min(x$n.I-c(0,x$n.I[1:(x$k-1)]))<=0) return(gsReturnError(x,errcode=11.3,errmsg="n.I must be an increasing, positive sequence"))
		if (x$maxn.IPlan<=0) x$maxn.IPlan<-max(x$n.I)
		else if (x$test.type<3 && is.character(x$sfu)) return(gsReturnError(x,errcode=11.31,
			errmsg="maxn.IPlan can only be > 0 if spending functions are used for boundaries"))
		x$timing<-x$n.I[1:(x$k-1)]/x$maxn.IPlan
		if (x$n.I[x$k-1]>=x$maxn.IPlan) return(gsReturnError(x,errcode=12,errmsg="maxn.IPlan must be > n.I[k-1]"))
	}
