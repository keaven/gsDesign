...
#                    gsUpdate:      adjust boundaries when timing of analyses changes for
#                                   a group sequential design created using a spending function
...
# make note of equal spacing
	eqspace<-(length(x$timing)==1 && x$timing==1)
...
# if maxn.IPlan was input, that should set the final analysis timing
# also: reset timing now that beta-spending has been set
      if (x$maxn.IPlan>0) x$timing<-x$n.I/x$n.I[x$k]
...
{	# set lower bound
	...
	{	if (x$upper$name=="WT") Delta<- x$upper$param
		else if (x$upper$name=="Pocock") Delta<-0.5
		else Delta<-0
		c<-WT(Delta,x$alpha,a,x$timing,x$tol,x$r)
...
{	if (max(x$upper$name==c("WT","Pocock","OF"))>0)
	...
	{	falsepos<-x$upper$spend
		...
		if (x$test.type==2 || x$astar < 1-x$alpha) sym<-1
		else sym<-0
		# set information to get desired power (only if sample size is being computed)
		if (max(x$n.I)==0)
		...
		else x2<-gsbound(I=x$n.I,trueneg=trueneg,falsepos=falsepos,tol=x$tol,r=x$r)
		x$upper$bound<-x2$b
		x$lower$bound<-x2$a
	}
	x$theta<-c(0,x$delta)
...
gsDType3<-function(x)
{	if (max(x$n.I)==0) return(gsDType3ss(x))
	else return(gsDType3a(x))
}
...
gsDType4<-function(x)
{	if (max(x$n.I)==0) return(gsDType4ss(x))
	else return(gsDType4a(x))
}
...
{	# set I0, desired false positive and false negative rates
	...
	# get lower bound
  	x2<-gsbound1(theta= -x$delta, I=x$n.I, a=-x1$b, probhi=falseneg, tol=x$tol, r=x$r)
	if (-x2$b[x2$k]> x1$b[x1$k]-x$tol) x2$b[x2$k]<- -x1$b[x1$k]
...
{	I<-time*Imax
	x<-gsprob(theta,I,a,b,r)
      # debugging line
	# print(c("I=",I,"probhi=",sum(x$probhi),"beta=",beta))
	beta-1+sum(x$probhi)
}
...
{   k<-length(I)
    ...
    if(symmetric==0) x$a[x$k]<-x$b[x$k]
...
gsbound1<-function(theta, I, a, probhi,tol=0.000001,r=18,printerr=1)
{   k<-as.integer(length(I))
...
{	k<-d$k
      n.I<-d$n.I
      if (d$test.type!=1) a<-d$lower$bound
	else a<-array(-20,k)
      b<-d$upper$bound
      r<-d$r
      ntheta<-as.integer(length(theta))
	...
	phi<-as.double(c(1:(k*ntheta)))
      ...
      d$theta<-theta
      d$upper$prob<-phi
	if (d$test.type!=1) d$lower$prob<-plo
	return(d)
}
...
{	if (k==0)
	...
	if (k <= 0 || k > 30)
	{	errcode<-1.01
		errmsg<-"k must be > 0 and <= 30"
	}
	if (!is.real(theta))
	{	errcode<-2
		errmsg<-"theta must be real-valued"
	}
	else if (k != length(n.I))
	{	errcode<-3
		errmsg<-"length of n.I != k"
	}
	else if (k != length(a))
	{	errcode<-4
		errmsg<-"length of a must be k"
	}
	else if (k != length(b))
	{	errcode<-5
		errmsg<-"length of b must be k"
	}
	if (r<1 || r>80)
	{	errcode<-11
		errmsg<-"Input value for r must be integer from 1 to 80, inclusive"
	}
	else
	{     phi<-as.double(c(1:(k*ntheta)))
      	plo<-as.double(c(1:(k*ntheta)))
		...
		x<-list(k=xx[[1]],theta=xx[[3]],n.I=xx[[4]],lower=list(bound=xx[[5]],prob=plo),
                    upper=list(bound=xx[[6]],prob=phi),en=en,r=r,errcode=errcode,errmsg=errmsg)
	}
	if (errcode > 0) x<-list(k=k,theta=theta,n.I=n.I,lower=list(bound=a),upper=list(bound=b),r=r,errcode=errcode,errmsg=errmsg)
