...
# compute final n per group assuming no adaptation
	...
	if (length(nnewe)==1) nnewe<-array(nnewe,TrialPar$nsim)
	nnewc<-ceiling(x$gsx$n.I[x$gsx$k]/(1+x$ratio)-x$nc)
	if (length(nnewc)==1) nnewc<-array(nnewc,TrialPar$nsim)
...
# to estimated value at interim
	if (thetacp== -100) thetacp<-thetahat
...
# compute sample size to achieve desired conditional power
	...
	if (maxn > 0)
	{	maxn<-maxn-x$nc-x$ne
		if(length(maxn)==1) maxn<-array(maxn,x$nsim)
...
# simulate data for final stage
	...
	if (length(x$nc)==1) x$nc<-array(x$nc,TrialPar$nsim)
	x$nc[x$outcome==0]<-x$nc[x$outcome==0]+nnewc[x$outcome==0]
	if (length(x$ne)==1) x$ne<-array(x$ne,TrialPar$nsim)
