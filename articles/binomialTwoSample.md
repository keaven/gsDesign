...
rbind(
  ...
  lt_format(columns = c("lower", "upper", "Effect"), sig_digits = 3)
```
...
tibble("Z cutoff" = zcut, "p cutoff" = pnorm(zcut, lower.tail = FALSE)) |>
  lt() |>
  lt_format(columns = c("Z cutoff", "p cutoff"), sig_digits = 3) |>
...
ptab |>
  ...
  lt_format(columns = c("n", "Power"), sig_digits = 3) |>
...
binomialPowerTable(
  ...
  lt() |>
  lt_format(columns = "Type I error", sig_digits = 3) |>
