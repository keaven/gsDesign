...
not about post-design power sensitivity calculations. For an
introductory survival sample size workflow that is not tied to SAS
output, see
...
see
[`vignette("SurvivalOverview")`](https://keaven.github.io/gsDesign/devel/articles/SurvivalOverview.md).
...
[`gsSurv()`](https://keaven.github.io/gsDesign/devel/reference/nSurv.md)
to keep the input accrual rate and accrual duration fixed, then solve
the follow-up duration needed for the final group sequential event
requirement.
...
we begin with
[`gsDesign::gsSurv()`](https://keaven.github.io/gsDesign/devel/reference/nSurv.md).
The
[`gsSurv()`](https://keaven.github.io/gsDesign/devel/reference/nSurv.md)
function combines
...
[`gsSurv()`](https://keaven.github.io/gsDesign/devel/reference/nSurv.md)
can answer a different sample size question: keeping enrollment rates
and minimum follow-up fixed, how long must enrollment continue to power
the trial? The translation is `T = NULL` with `minfup` set to the fixed
