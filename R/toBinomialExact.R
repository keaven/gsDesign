...
#'
#' @param x Most often object of class \code{gsSurv}; i.e., an object generated by
#'   the \code{gsSurv()} function. Less commonly used but this can be an object of type \code{gsDesign}.
#' @param observedEvents If NULL (default), targeted timing of analyses will come from \code{x$n.I}. 
#' Otherwise, this should be vector of increasing positive integers with at most 1 value \code{>=} maximum events planned (\code{x$n.I[x$k]}) and 
...
#' @param alpha alpha-level for which efficacy bounds are to be computed; if NULL, this is taken from x$alpha.
#' 
#' @details
#' 
#' The exact binomial routine \code{gsBinomialExact} used to compute operating characteristics for the derived design
#' has requirements that may not be satisfied by the initial asymptotic approximation based on the input Lachin and Foulkes design input \code{x}. 
...
#' This will also include an object \code{init_approx} which represents initial asymptotic approximations of bounds. 
#' This includes a vector \code{a} for the lower (efficacy) bound and, if \code{test.type = 4}, a vector \code{b} for 
#' the upper (futility) bound.
#'
...
#' xx <- toBinomialExact(x, observedEvents = c(20,55,80))
#' xx
#' 
#' # Now confirm that alpha-spending is controlled at targeted level
#' # Event counts at planned analysis divided by final planned count
#' timing <- observedEvents / xx$n.I[x]
#' # Use spending function to show target
#' targeted_spend <- x$upper$sf(alpha = x$alpha, t = timing, param = x$upper$param)$spend
#' # Use gsBinomialExact with no futililty bound b to get actual cumulative spending
#' actual_spend <- cumsum(gsBinomialExact(k = xx$k, a = xx$lower$bound, b = xx$n.I + 1, theta = xx$theta[1], n.I = xx$n.I)$lower$prob)
#' targeted_spend
#' actual_spend
#' 
#' # Now compute bounds for an alternate alpha-level
#' xx.3 <- toBinomialExact(x, observedEvents = c(20,55,80), alpha = .3)
#' xx.3
#' 
toBinomialExact <- function(x, observedEvents = NULL, alpha = NULL){
  # Note that a gsSurv object is also a gsDesign object (i.e., inherits characteristics)
  if (!inherits(x, "gsDesign")) stop("toBinomialExact must have class gsSurv or gsDesign as input")
  ...
  if(is.null(alpha)){alpha <- x$alpha
          ...
          stop("toBinomialExact: alpha must be NULL or a scalar of length 1 in the open interval (0,1)")
  }
  ...
  alpha_spend <- x$upper$sf(alpha = alpha, t = timing, param = x$upper$param)$spend
  if (x$test.type != 1) {
