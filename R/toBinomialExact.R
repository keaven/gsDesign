...
#' Otherwise, this should be a vector of increasing positive integers, of length
#' at least 2 for a sequential design or length 1 for a fixed design.
...
#' 
#' @details
#' Fixed survival designs (\code{gsSurv(k = 1)}) are converted using the exact
#' lower binomial tail under \code{hr0}. The largest integer efficacy cutoff
#' with probability no greater than the available alpha spending is selected;
#' discreteness may reduce achieved power. A single \code{observedEvents} value
...
toBinomialExact <- function(x, observedEvents = NULL, alpha = NULL, usTime = NULL, lsTime = NULL, maxSpend = FALSE) {
  ...
  if(is.null(observedEvents)){
    ...
    if (!length(observedEvents) || !isInteger(observedEvents)) stop("toBinomialExact: observedEvents must be a vector of increasing positive integers")
    ...
    if (k < 2 && x$k != 1) stop("toBinomialExact: must have at least 2 values in observedEvents for a sequential design")
  }
  if (x$k == 1L) {
    if (k != 1L) stop("toBinomialExact: a fixed design requires one observed event count")
    timing <- resolveSpendingTime(usTime, pmin(counts / planned_final_events, 1), 1L, "usTime")
    if (maxSpend) timing <- 1
    spend <- xx$upper$sf(alpha, timing, xx$upper$param)$spend
    p0 <- x$hr0 * x$ratio / (1 + x$hr0 * x$ratio)
    p1 <- x$hr * x$ratio / (1 + x$hr * x$ratio)
    a <- qbinom(spend, counts, p0)
    if (pbinom(a, counts, p0) > spend) a <- a - 1
    out <- gsBinomialExact(k = 1L, theta = c(p0, p1), n.I = counts,
                          a = a, b = counts + 1)
    out$init_approx <- list(a = qbinom(pnorm(-xx$upper$bound), counts, p0) - 1)
    out$test.type <- 1L
    out$alpha <- alpha
    out$beta <- x$beta
    out$testUpper <- TRUE
    out$usTime <- timing
    out$maxn.IPlan <- planned_final_events
    out$maxSpend <- maxSpend
    out$ratio <- x$ratio
    out$call <- match.call()
    class(out) <- c("gsBinomialExactSpending", class(out))
    return(out)
  }
