...
gsPP <- function(x, i=1, zi=0, theta=c(0,3), wgts=c(.5,.5), r=18, total=TRUE){   if (!(methods::is(x, "gsProbability") || methods::is(x, "gsDesign")))
    ...
    test.type <- ifelse(methods::is(x, "gsProbability"), 3, x$test.type)
    checkmate::checkScalar(i, "integer", c(1, x$k-1))
    checkmate::checkScalar(zi, "numeric", c(-Inf,Inf), c(FALSE, FALSE))
    checkmate::checkVector(wgts, "numeric", c(0, Inf), c(TRUE, FALSE))
    checkmate::checkVector(theta, "numeric", c(-Inf,Inf), c(FALSE, FALSE))
    checkLengths(theta, wgts)
    checkmate::checkScalar(r, "integer", c(1, 80))
...
gsPI<-function(x, i=1, zi=0, j=2, level=.95, theta=c(0,3), wgts=c(.5,.5)){
   ...
   checkmate::checkScalar(i, "integer", c(1, x$k-1))
   checkmate::checkScalar(j, "integer", c(i+1, x$k))
   checkmate::checkScalar(zi, "numeric", c(-Inf,Inf), c(FALSE,FALSE))
   checkmate::checkVector(wgts, "numeric", c(0, Inf), c(TRUE, FALSE))
   checkmate::checkVector(theta, "numeric", c(-Inf,Inf), c(FALSE, FALSE))
   checkLengths(theta, wgts)
   ...
   post <- post/sum(post)
   lower <- stats::uniroot(f=postfn, interval=c(-20,20), PP=1-(1-level)/2, 
                    d=x, i=i, zi=zi, j=j, theta=theta, wgts=post)$root
   if (level==0) return(lower)
   upper <- stats::uniroot(f=postfn, interval=c(-20,20), PP=(1-level)/2,
                    d=x, i=i, zi=zi, j=j, theta=theta, wgts=post)$root
   c(lower,upper)
}
...
gsBoundCP <- function(x, theta="thetahat", r=18){   if (!(methods::is(x, "gsProbability") || methods::is(x, "gsDesign")))
    ...
    checkmate::checkScalar(r, "integer", c(1, 70))
...
gsPosterior <- function(x=gsDesign(), i=1, zi=NULL, prior=normalGrid(),r=18){   if (is.null(prior$gridwgts)) prior$gridwgts <- array(1,length(prior$z))
    ...
    checkmate::checkVector(prior$gridwgts, "numeric", c(0, Inf), c(TRUE, FALSE))
    checkmate::checkVector(prior$density, "numeric", c(0, Inf), c(TRUE, FALSE))
    checkmate::checkVector(prior$z, "numeric", c(-Inf,Inf), c(FALSE, FALSE))
    if (!(methods::is(x, "gsProbability") || methods::is(x, "gsDesign")))
      stop("gsPosterior: x must have class gsDesign or gsProbability")
    test.type <- ifelse(methods::is(x, "gsProbability"), 3, x$test.type)
    checkmate::checkScalar(i, "integer", c(1, x$k-1))
    if (is.null(zi)) zi <- c(x$lower$bound[i],x$upper$bound[i])
    else checkmate::checkVector(zi, "numeric", c(-Inf,Inf), c(FALSE,FALSE))
