...
xtable.gsSurv <- function(x, caption = NULL, label = NULL, align = NULL, digits = NULL,
  ...
  if (x$test.type != 1) fut[5 * (0:(k - 1)) + 2] <- as.character(round(gsDesign::gsHR(z = x$lower$bound, i = 1:k, x, ratio = x$ratio) * x$hr0, 2))
  eff[5 * (0:(k - 1)) + 2] <- as.character(round(gsDesign::gsHR(z = x$upper$bound, i = 1:k, x, ratio = x$ratio) * x$hr0, 2))
