...
testthat::test_that("test.ciBinomial.misc", {
  testthat::expect_equal(as.numeric(gsDesign::ciBinomial(
    ...
    -0.1658,
    0.2843608
  ), info = "ciBinomial numeric check #1 failed (M&N example 4")
  testthat::expect_equal(as.numeric(gsDesign::ciBinomial(
    x1 = 72, x2 = 20, n1 = 756,
    ...
    0.0344,
    0.0868
  ), info = "ciBinomial numeric check #2 failed")
  testthat::expect_equal(as.numeric(gsDesign::ciBinomial(
    x1 = 10, x2 = 20, n1 = 10,
...
testthat::test_that("test.nBinomial.misc", {
  ...
  c(62.88, 41.92),
  info = "nBinomial check #2 failed (F&M, Table 1)"
  )
  testthat::expect_equal(as.numeric(gsDesign::nBinomial(
  ...
  c(173.2214, 259.8322),
  info = "nBinomial check #3 failed (F&M, Table 1)"
  )
  testthat::expect_equal(as.numeric(gsDesign::nBinomial(
  ...
  c(900.57, 1350.85),
  info = "nBinomial check #6 failed (OR scale)"
  )
})
...
testthat::test_that("test.simBinomial.misc", {
  ...
  0.813,
  info = "simBinomial check #1 failed (F&M bottom p 1451)"
  )
  testthat::expect_equal(sum(-stats::qnorm(0.05) < gsDesign::simBinomial(
    p1 = 0.1, p2 = 0.1,
  ...
  0.916,
  info = "simBinomial check #2 failed (F&M Table 1)"
  )
  testthat::expect_equal(sum(-stats::qnorm(0.05) < gsDesign::simBinomial(
    p1 = 0.25, p2 = 0.05,
  ...
  0.9078,
  info = "simBinomial check #6 failed (F&M Table 1)"
  )
})
...
testthat::test_that("test.testBinomial.numerics", {
  ...
  c(1.959962, -1.959865),
  info = "Error in testBinomial example 1 (M&N example 4)"
  )
  testthat::expect_equal(gsDesign::testBinomial(
  ...
  c(1.96, -1.96),
  info = "Error in testBinomial example 2 (M&N example 6)"
  )
  testthat::expect_equal(gsDesign::testBinomial(
    ...
    1.964856,
    -1.963414
  ), info = "Error in testBinomial example 3 (p 27, Lachin)")
  testthat::expect_equal(gsDesign::testBinomial(
    x1 = 53, x2 = 40, n1 = 100, n2 = 100,
    ...
    1.965205,
    -1.96461
  ), info = "Error in testBinomial example 4 (p 27, Lachin)")
})
