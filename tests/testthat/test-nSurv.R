...
testthat::test_that("Checking consistency of Schoenfeld approximations", {
  z <- gsDesign::hrn2z(hr = .7, n = 100, ratio = 1.5)
  hr <- gsDesign::zn2hr(z = -z, n = 100, ratio = 1.5)
  n <- gsDesign::hrz2n(z = -z, hr = .7, ratio = 1.5)
...
testthat::test_that("Checking consistency nEvents power vs sample size", {
  ss <- gsDesign::nEvents(hr = .7, tbl = TRUE)
  ne <- gsDesign::nEvents(hr = .7)
  pwr <- gsDesign::nEvents(hr = .7, n = ne, tbl = TRUE)
...
testthat::test_that("nSurv matches rpact for Schoenfeld and Freedman", {
  ...
  for (ratio in c(0.5, 1, 2)) {
    ...
    testthat::expect_equal(
      ns_schoen$n, rp_schoen$maxNumberOfSubjects[1], tolerance = 1e-6
    )
    testthat::expect_equal(
      ns_schoen$d, rp_schoen$maxNumberOfEvents[1], tolerance = 1e-6
    )
  }
  ...
  for (ratio in c(0.5, 1, 2)) {
    ...
    testthat::expect_equal(
      ns_freed$d, rp_freed$maxNumberOfEvents[1], tolerance = 1e-6
    )
    testthat::expect_equal(
      ns_freed$n, rp_freed$maxNumberOfSubjects[1], tolerance = 1e-6
    )
  }
})
